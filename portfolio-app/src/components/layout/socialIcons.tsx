...
/*
 * socialIcons — the single source of truth for the four real social platforms'
 * accessible LABELS and inline-SVG GLYPHS (github / facebook / youtube /
 * instagram). Rendered inside a shared `<svg viewBox="0 0 24 24"
 * fill="currentColor">` wrapper by BOTH footer consumers:
 *   - SocialBar   (Footer "normal" variant — every page)
 *   - SocialOrbit (Footer "landing" variant — Home)
 *
 * Extracted from those two files, which held byte-identical copies of these
 * dictionaries — so a corrected glyph or a new platform had to be edited twice
 * or the two footers silently diverged. Orbit-only decorative badges
 * (slack/gemini/chatgpt/telegram/whatsapp) stay in SocialOrbit; they are not
 * shared and not part of the real-platform contract.
 *
 * The exhaustive `Record<SocialMedia['platform'], …>` type makes adding a
 * platform a single compile-checked edit HERE that both consumers pick up.
 */

export const SOCIAL_LABELS: Record<SocialMedia['platform'], string> = {
  facebook: 'Facebook',
  instagram: 'Instagram',
  github: 'GitHub',
  youtube: 'YouTube',
...
export const SOCIAL_ICONS: Record<SocialMedia['platform'], React.ReactNode> = {
  github: (
